...
const parseTsv = (dataString, columnNames) => {
  return new Promise((resolve, reject) => {
    ...
    const cleanedDataString = dataString
      .split("\n")
...
const extractCleanDataBlock = (potentialDataLines, expectedColumnNames) => {
  const cleanLines = [];
  // The critical check here is based on the *first column* of the expected data.
  // If the first cell of a row is empty or matches a known non-data pattern, it signals the end.
  const firstExpectedColumnHeader = expectedColumnNames[0]; // This variable is not used but kept for clarity if needed later

  for (const line of potentialDataLines) {
    const trimmedLine = line.trim();
    // If the line is empty or starts with patterns typically found in legends/instructions
    if (
      trimmedLine.length === 0 ||
      trimmedLine.toLowerCase().startsWith("légende") ||
      trimmedLine.toLowerCase().startsWith("instructions") ||
      trimmedLine.toLowerCase().startsWith("copiez toutes les lignes") // Specific check for the instruction
    ) {
      break; // Stop at the first non-data line
    }

    // Additional check: If the line has less than a certain number of tabs, it might not be data.
    // Or if the first "cell" is empty.
    const cells = line.split("\t");
    if (cells[0].trim() === "") {
      // If the very first cell is empty, it's likely not data
      break;
    }
 ...
 * This function precisely identifies each section's header, extracts the data
 * that follows, and then sends it to `parseTsv` with explicit column names.
 * It is designed to ignore any non-table content (like legends or stray text)
 * that appears outside of the clearly defined data blocks.
...
const parsePastedExcelData = async (pastedData) => {
  ...
  // Step 1: Find the exact header row indices for both sheets within the full pasted data
  ...
  // If neither header is found, throw an error immediately
  ...
  // Step 2: Extract the pure data blocks based on detected headers
  // This logic ensures only the lines immediately following a header and before another header (or end of paste) are considered data.
  if (headerHeaderIndex !== -1 && lineItemHeaderIndex !== -1) {
    if (headerHeaderIndex < lineItemHeaderIndex) {
      // Scenario: Header sheet block comes before Line Item sheet block
      // Potential header data lines are from after its header up to the line item header
      ...
      // Potential line item data lines are from after its header to the end of pasted data
      ...
      // Scenario: Line Item sheet block comes before Header sheet block
      // Potential line item data lines are from after its header up to the header sheet header
      const potentialLineItemDataLines = rawLines.slice(
        lineItemHeaderIndex + 1,
        headerHeaderIndex
      );
      ...
      // Potential header data lines are from after its header to the end of pasted data
    ...
    // Only Header section found (data starts after its header, goes to end of paste)
    const potentialHeaderDataLines = rawLines.slice(headerHeaderIndex + 1);
    const cleanHeaderDataLines = extractCleanDataBlock(
      potentialHeaderDataLines,
      INVOICE_HEADER_COLUMN_NAMES
    ...
    // Only Line Item section found (data starts after its header, goes to end of paste)
  ...
  // Filter out any line item rows that are effectively empty (e.g., just tabs from empty Excel rows)
  // This is a redundant but safe check if extractCleanDataBlock already did its job well.
  parsedLineItemData = parsedLineItemData.filter((row) =>
    row["ID Ligne (Article)"]?.trim()
  );

  // Map header data to the expected xmlPath/value format
  // parseTsv with `columns:columnNames` means the parsed objects already have the correct keys.
...
const validateField = (value, rules, fieldName, errors) => {
  ...
  if (trimmedValue) {
    // Only apply other rules if value is present
    ...
    if (rules.type === "date" && trimmedValue) {
      ...
      if (!/^\d{6}$/.test(trimmedValue)) {
        errors.push(
          `${fieldName}: Le format de date doit être 'ddMMyy' (ex: 250624).`
        );
        isValid = false;
      } else {
        ...
        if (day < 1 || day > 31 || month < 1 || month > 12) {
          errors.push(
            `${fieldName}: Jour ou mois invalide dans '${trimmedValue}'.`
          );
          isValid = false;
        } else {
          ...
          if (
            ...
            errors.push(
              `${fieldName}: Date '${trimmedValue}' invalide (ex: 31 février).`
            );
            isValid = false;
          }
        }
    ...
    if (
      rules.type === "monetaryAmount" &&
      trimmedValue &&
      !/^-?[0-9]{1,15}(\.[0-9]{1,5})?$/.test(trimmedValue)
    ) {
      errors.push(
        `${fieldName}: Le format du montant '${trimmedValue}' est invalide. Attendu: NNNN.NNNNN (max 15 chiffres entiers, max 5 décimales).`
      );
      isValid = false;
    }
  }
...
const validateInvoiceData = async (data, selectedCompany) => {
    ...
    switch (item.xmlPath) {
      ...
      case "InvoiceBody.Bgm.DocumentType.@code":
        ...
        if (
          ...
          invoiceData.TEIF.InvoiceBody.Bgm.DocumentType._attributes.code =
            value;
          invoiceData.TEIF.InvoiceBody.Bgm.DocumentType._value =
            lookupMaps.documentTypeMap.get(value) || "";
        }
        break;
      ...
      case "InvoiceBody.PytSection.PytSectionDetails.0.PytFii.AccountHolder.AccountNumber":
        // FIX: Increased maxLength from 20 to 35 to accommodate IBAN example
        validateField(
          value,
          { maxLength: 35 },
          "Numéro de Compte Bancaire",
      ...
      case "InvoiceBody.InvoiceTax.InvoiceTaxDetails.0.Tax.TaxDetails.TaxRate":
        if (
          ...
          // Pattern for percentage, e.g., 19.00
      ...
      default:
        // Removed problematic error logging for pre-filled fields.
        // Any xmlPath not explicitly handled by a case is simply ignored.
        break;
    }
  });
  ...
  if (receiverIdentifierType && receiverIdentifierValue) {
    ...
    // FIX: Adjusted regex for I-01 to be more flexible, matching common numeric formats and strict format
    if (
      receiverIdentifierType === "I-01" &&
      !/^(?:[0-9]{7}[A-Z][0-9]{3}|[0-9]{7,13})$/i.test(receiverIdentifierValue)
    ) {
      errors.push(
        `Identifiant Destinataire: Le format du Matricule Fiscal Tunisien (Type I-01) est invalide pour '${receiverIdentifierValue}'. Attendu: 7 chiffres + 1 lettre + 3 chiffres (Ex: 1234567A001) OU 7 à 13 chiffres.`
      );
    } else if (
      receiverIdentifierType === "I-02" &&
      !/^[0-9]{8}$/.test(receiverIdentifierValue)
    ) {
      errors.push(
        `Identifiant Destinataire: Le format du CIN (Type I-02) est invalide pour '${receiverIdentifierValue}'.`
      );
    } else if (
      receiverIdentifierType === "I-03" &&
      !/^[0-9]{9}$/.test(receiverIdentifierValue)
    ) {
      errors.push(
        `Identifiant Destinataire: Le format de la Carte de Séjour (Type I-03) est invalide pour '${receiverIdentifierValue}'.`
      );
    }
    ...
    errors.push(
      "L'identifiant et le type de l'identifiant du destinataire sont requis."
    );
  }

  if (!invoiceData.TEIF.InvoiceBody.Bgm.DocumentIdentifier)
    errors.push("Le numéro de facture est requis.");
  if (!invoiceData.TEIF.InvoiceBody.Bgm.DocumentType._attributes.code)
  ...
  if (!receiverName || !receiverAddress || !receiverCity || !receiverCountry)
    errors.push(
      "Le nom, l'adresse, la ville et le pays du récepteur sont requis."
    );
  ...
  invoiceData.TEIF.InvoiceBody.PartnerSection.PartnerDetails.push({
    ...
    Nad: {
      ...
      PartnerName: {
        _value: selectedCompany.name,
        _attributes: { nameType: "Qualification" },
      },
      PartnerAdresses: [
        {
          ...
          Country: {
            _value: selectedCompany.country,
            _attributes: { codeList: "ISO_3166-1" },
          },
        },
      ],
      CtaSection: {
        Contact: selectedCompany.contact_name
          ? {
              ContactIdentifier: "CONTACT1",
              ContactName: selectedCompany.contact_name,
            }
          : null,
        Communication: [],
      },
  ...
  if (receiverName && receiverAddress && receiverCity && receiverCountry) {
    ...
    const receiverPartnerDetailsIndex =
      invoiceData.TEIF.InvoiceBody.PartnerSection.PartnerDetails.length - 1;
    if (receiverEmail) {
      invoiceData.TEIF.InvoiceBody.PartnerSection.PartnerDetails[
        receiverPartnerDetailsIndex
      ].Nad.CtaSection.Communication.push({
        ComMeansType: "I-102",
    ...
    invoiceData.TEIF.InvoiceBody.PytSection.PytSectionDetails.push({
      ...
      PytFii:
        bankAccountNumber || bankName
          ? {
              AccountHolder: bankAccountNumber
                ? { AccountNumber: bankAccountNumber }
                : null,
              InstitutionIdentification: bankName
                ? {
                    BranchIdentifier: "BRANCH1",
                    InstitutionName: bankName,
                    _attributes: { nameCode: "SWIFT" },
                  }
                : null, // Assuming SWIFT as nameCode
  ...
  for (const [index, lineItem] of parsedLineItemData.entries()) {
    ...
    // Ensure that lineItem has the expected keys from the header.
    // If a row is entirely malformed or empty, it might not have these.
    ...
    const lineTaxTypeCode =
      lineItem["Code Type Taxe Ligne (Ex: I-1602)"]?.trim();
    ...
    if (
      ...
      continue; // Skip entirely empty or header-like rows that made it through initial filtering
    }
    ...
    if (
      !validateField(
        itemIdentifier,
        { required: true, maxLength: 35 },
        `Ligne ${lineNumber}: ID Ligne (Article)`,
        itemErrors
      )
    ) {
    }
    ...
    if (
      !validateField(
        itemDescription,
        { required: true, maxLength: 500 },
        `Ligne ${lineNumber}: Description Article`,
        itemErrors
      )
    ) {
    }
    ...
    if (
      !validateField(
        quantityStr,
        { required: true, type: "monetaryAmount" },
        `Ligne ${lineNumber}: Quantité`,
        itemErrors
      )
    ) {
      // Using monetaryAmount for quantity to allow decimals and limit size
    ...
    if (
      !validateField(
        ...
        `Ligne ${lineNumber}: Unité de Mesure`,
        itemErrors
      )
    ) {
    }
    ...
    if (
      !validateField(
        unitPriceHTStr,
        { required: true, type: "monetaryAmount" },
        `Ligne ${lineNumber}: Prix Unitaire HT`,
        itemErrors
      )
    ) {
      unitPriceHT = 0;
    ...
    if (
      !validateField(
        lineTaxTypeCode,
        { required: true, enum: Array.from(lookupMaps.taxTypeMap.keys()) },
        `Ligne ${lineNumber}: Code Type Taxe Ligne`,
        itemErrors
      )
    ) {
    }
    ...
    if (
      !validateField(
        ...
        `Ligne ${lineNumber}: Taux de Taxe Ligne`,
        itemErrors
      )
    ) {
      lineTaxRate = 0;
    ...
    if (itemErrors.length === 0) {
      // Only calculate if line item is valid
      lineItemCount++;
      ...
      invoiceData.TEIF.InvoiceBody.LinSection.Lin.push({
        ...
        LinQty: {
          Quantity: {
            _value: quantity.toFixed(5),
            _attributes: { measurementUnit: measurementUnit },
          }, // Quantity formatted to 5 decimal places
        },
        LinTax: {
          TaxTypeName: {
            _value: lookupMaps.taxTypeMap.get(lineTaxTypeCode) || "",
            _attributes: { code: lineTaxTypeCode },
          },
          TaxDetails: {
          ...
          MoaDetails: [
            {
              Moa: {
                Amount: {
                  _value: lineNet.toFixed(5),
                  _attributes: { currencyIdentifier: "TND" },
                },
                _attributes: {
  ...
  if (lineItemCount === 0 && errors.length === 0) {
    errors.push(
      "Aucune ligne de facture valide détectée. Une facture doit contenir au moins une ligne."
    );
  }
  ...
  if (errors.length === 0) {
    ...
    if (invoiceTaxTypeCode && invoiceTaxRate !== undefined) {
      invoiceData.TEIF.InvoiceBody.InvoiceTax.InvoiceTaxDetails.push({
        Tax: {
          TaxTypeName: {
            _value: lookupMaps.taxTypeMap.get(invoiceTaxTypeCode) || "",
            _attributes: { code: invoiceTaxTypeCode },
          },
        ...
        AmountDetails: [
          {
            Moa: {
              Amount: {
                _value: totalTaxAmount.toFixed(5),
                _attributes: { currencyIdentifier: "TND" },
              },
              _attributes: {
                currencyCodeList: "ISO_4217",
      ...
      errors.push(
        "Le code et le taux de la taxe au niveau de la facture sont requis pour le calcul global."
      );
    }
  }
  ...
  return {
    ...
    rowCount: parsedLineItemData.length,
  };
};
...
async function getLookupMaps() {
  ...
  return {
    ...
    partnerIdentifierTypeMap: new Map(
      partnerIdentifierTypes.map((item) => [item.code, item.description])
    ),
    dateFunctionMap: new Map(
      dateFunctions.map((item) => [item.code, item.description])
    ),
    partnerFunctionMap: new Map(
      partnerFunctions.map((item) => [item.code, item.description])
    ),
    taxTypeMap: new Map(taxTypes.map((item) => [item.code, item.description])),
    paymentTermsTypeMap: new Map(
      paymentTermsTypes.map((item) => [item.code, item.description])
    ),
    paymentMeansMap: new Map(
      paymentMeans.map((item) => [item.code, item.description])
    ),
    communicationMeansMap: new Map(
      communicationMeans.map((item) => [item.code, item.description])
    ),
    freeTextSubjectsMap: new Map(
      freeTextSubjects.map((item) => [item.code, item.description])
    ),
  };
}
...
exports.validateExcelData = async (req, res) => {
  ...
  const { companyId, pastedData } = req.body; // pastedData is now a string from textarea
  let errors = [];

  try {
    if (!companyId || !pastedData) {
      errors.push(
        "Company ID et données collées sont requis pour la validation."
      );
      return res
        .status(400)
        .json({
    ...
    errors.push(
      `Erreur interne du serveur lors de la validation: ${error.message}`
    );
    res
      .status(500)
      .json({
...
exports.generateXmlInvoice = async (req, res) => {
  ...
  try {
    ...
    const fileName = `invoice_${
      invoice.TEIF.InvoiceBody.Bgm.DocumentIdentifier
    }_${Date.now()}.xml`;
    ...
    res
      ...
      .json({
        success: false,
        message: "Server error during XML generation",
        errors: errors,
      });
  }
};
